...
export default function AdminDashboard() {
  ...
  const filteredApps = filter === 'all' 
    ? applications 
    : applications.filter(app => app.status === filter || (filter === 'submitted' && app.status === 'pending'))
        ...
        <div className="modal-overlay" onClick={() => setSelectedApp(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              {['submitted', 'pending', 'processed'].includes(selectedApp.status) && (
                  ...
                  {!action ? (
                    ...
                    <div className="action-form">
                      <h4>{
                        ...
                        action === 'process' ? 'Process Application' : 'Reject Application'
                      }</h4>
                      <div className="form-group">
                        <label>
                      ...
                      <div className="button-group">
                        <button
                          className={
                            action === 'approve' ? 'btn btn-success' :
                            action === 'process' ? 'btn btn-primary' : 'btn btn-danger'
                          }
                          onClick={
                            action === 'approve' ? handleApprove :
                            action === 'process' ? async () => {
                              try {
                                await adminAPI.updateStatus(selectedApp.applicationId, 'processed', notes)
                                setSelectedApp(null)
                                setAction(null)
                                setNotes('')
                                fetchData()
                        ...
                        >
                          {action === 'approve' ? 'Confirm Approval' :
                           action === 'process' ? 'Confirm Processed' : 'Confirm Rejection'}
                        </button>
                        <button
                          className="btn btn-secondary"
                          onClick={() => {
